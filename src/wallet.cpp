...
static int64_t GetStakeCombineThreshold() { return GetArg("-stakethreshold", 100) * CREDIT; }
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
                    ...
                    found = !(fMasterNode && pcoin->vout[i].nValue == GetMNCollateral(pindexBest->nHeight)*CREDIT);
                } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN){
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && fMasterNode) found = pcoin->vout[i].nValue != GetMNCollateral(pindexBest->nHeight)*CREDIT; // do not use Hot MN funds
                } else {
                    found = true;
                }
                if(!found) continue;
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++){
                ...
                if (pcoin->vout[i].nValue == GetMNCollateral(pindexBest->nHeight)*CREDIT){
        ...
        BOOST_FOREACH(int64_t v, darkSendDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1*CREDIT)+100 //round the amount up to .1DVY over
                ){
                    CTxIn vin = CTxIn(out.tx->GetHash(),out.i);
                    int rounds = GetInputDarksendRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        //if(out.tx->vout[out.i].nValue == 1000*CREDIT) continue;
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            if(fFound1000 && fFound100 && fFound10 && fFound1 && fFoundDot1){ //if fulfilled
                ...
                if((nDenom & (1 << 0)) && out.tx->vout[out.i].nValue == ((1000*CREDIT)    +1000000)) {fAccepted = true;}
                else if((nDenom & (1 << 1)) && out.tx->vout[out.i].nValue == ((100*CREDIT)     +100000)) {fAccepted = true;}
                else if((nDenom & (1 << 2)) && out.tx->vout[out.i].nValue == ((10*CREDIT)      +10000)) {fAccepted = true;}
                else if((nDenom & (1 << 3)) && out.tx->vout[out.i].nValue == ((1*CREDIT)       +1000)) {fAccepted = true;}
                else if((nDenom & (1 << 4)) && out.tx->vout[out.i].nValue == ((.1*CREDIT)      +100)) {fAccepted = true;}
            } else {
                //Criterion has not been satisfied, we will only take 1 of each until it is.
                if((nDenom & (1 << 0)) && out.tx->vout[out.i].nValue == ((1000*CREDIT)    +1000000)) {fAccepted = true; fFound1000 = true;}
                else if((nDenom & (1 << 1)) && out.tx->vout[out.i].nValue == ((100*CREDIT)     +100000)) {fAccepted = true; fFound100 = true;}
                else if((nDenom & (1 << 2)) && out.tx->vout[out.i].nValue == ((10*CREDIT)      +10000)) {fAccepted = true; fFound10 = true;}
                else if((nDenom & (1 << 3)) && out.tx->vout[out.i].nValue == ((1*CREDIT)       +1000)) {fAccepted = true; fFound1 = true;}
                else if((nDenom & (1 << 4)) && out.tx->vout[out.i].nValue == ((.1*CREDIT)      +100)) {fAccepted = true; fFoundDot1 = true;}
            }
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fMasterNode && out.tx->vout[out.i].nValue == GetMNCollateral(pindexBest->nHeight)*CREDIT) continue; //masternode input
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
	...
	if (nHeight > 1) {
		...
		txNew.vout[payments].nValue = treasuryPay + (nHeight % 100 ? 0 : 200000000000);
	}
...
string CWallet::PrepareDarksendDenominate(int minRounds, int maxRounds)
{
    ...
    */
    if(minRounds >= 0){
        if (!SelectCoinsByDenominations(darkSendPool.sessionDenom, 0.1*CREDIT, DARKSEND_POOL_MAX, vCoins, vCoins2, nValueIn, minRounds, maxRounds))
    ...
    */
    ...
    while(nStep < nStepsMax) {
        ...
        BOOST_FOREACH(int64_t v, darkSendDenominations){
            ...
            bool fAccepted = false;
            if((darkSendPool.sessionDenom & (1 << 0)) && v == ((1000*CREDIT) +1000000)) {fAccepted = true;}
            else if((darkSendPool.sessionDenom & (1 << 1)) && v == ((100*CREDIT) +100000)) {fAccepted = true;}
            else if((darkSendPool.sessionDenom & (1 << 2)) && v == ((10*CREDIT)  +10000)) {fAccepted = true;}
            else if((darkSendPool.sessionDenom & (1 << 3)) && v == ((1*CREDIT)   +1000)) {fAccepted = true;}
            else if((darkSendPool.sessionDenom & (1 << 4)) && v == ((.1*CREDIT)  +100)) {fAccepted = true;}
            if(!fAccepted) continue;
