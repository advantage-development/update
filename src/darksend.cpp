...
// The mainfunctions.object for accessing darksend
...
int CDarksendPool::GetDenominationsByAmount(int64_t nAmount, int nDenomTarget){
    ...
    BOOST_REVERSE_FOREACH(int64_t v, darkSendDenominations){
        if(nDenomTarget != 0){
            ...
            if((nDenomTarget & (1 << 0)) && v == ((1000*CREDIT) +1000000)) {fAccepted = true;}
            else if((nDenomTarget & (1 << 1)) && v == ((100*CREDIT) +100000)) {fAccepted = true;}
            else if((nDenomTarget & (1 << 2)) && v == ((10*CREDIT) +10000)) {fAccepted = true;}
            else if((nDenomTarget & (1 << 3)) && v == ((1*CREDIT)  +1000)) {fAccepted = true;}
            else if((nDenomTarget & (1 << 4)) && v == ((.1*CREDIT) +100)) {fAccepted = true;}
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey){
    ...
    if(GetTransaction(vin.prevout.hash, txVin, hash)){
        BOOST_FOREACH(CTxOut out, txVin.vout){
            if(out.nValue == GetMNCollateral(pindexBest->nHeight)*CREDIT){
