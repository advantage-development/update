...
private:
    ...
    bool SelectCoins(CAmount nTargetValue, unsigned int nSpendTime, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl *coinControl = NULL, AvailableCoinsType coin_type=ALL_CREDITS, bool useIX = false) const;
...
public:
    ...
    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed=true, const CCoinControl *coinControl = NULL, AvailableCoinsType coin_type=ALL_CREDITS, bool useIX = false) const;
    void AvailableCoinsMN(std::vector<COutput>& vCoins, bool fOnlyConfirmed=true, const CCoinControl *coinControl = NULL, AvailableCoinsType coin_type=ALL_CREDITS, bool useIX = false) const;
    ...
    bool CreateTransaction(const std::vector<std::pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, int32_t& nChangePos, std::string& strFailReason, const CCoinControl *coinControl=NULL, AvailableCoinsType coin_type=ALL_CREDITS, bool useIX=false);
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache=true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            ...
            if(fMasterNode && vout[i].nValue == GetMNCollateral(pindexBest->nHeight)*CREDIT) continue; // do not count MN-like outputs
