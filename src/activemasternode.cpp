...
// Copyright (c) 2018 Advantage developers
...
//
// Bootup the masternode, look for a 1,000,000 USDA input and register on the network
//
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    {
        if(out.tx->vout[out.i].nValue == GetMNCollateral(pindexBest->nHeight)*CREDIT) { //exactly
...
vector<COutput> CActiveMasternode::SelectCoinsMasternodeForPubKey(std::string collateralAddress)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        if(out.tx->vout[out.i].scriptPubKey == scriptPubKey && out.tx->vout[out.i].nValue == GetMNCollateral(pindexBest->nHeight)*CREDIT) { //exactly
        	filteredCoins.push_back(out);
        }
    }
    return filteredCoins;
