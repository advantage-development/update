...
CBlock* CreateNewBlock(CReserveKey& reservekey, bool fProofOfStake, int64_t* pFees)
{
    ...
    else
    {
        ...
        txNew.vin[0].scriptSig = (CScript() << nHeight) + CREDITBASE_FLAGS;
    ...
    int64_t nFees = 0;
	{
		...
		while (!vecPriority.empty())
		{
			...
			// transactions:
			if (!fSortedByFee &&
				((nBlockSize + nTxSize >= nBlockPrioritySize) || (dPriority < CREDIT * 144 / 250)))
			{
				fSortedByFee = true;
		...
		if (!fProofOfStake){
			...
			if (nHeight > 1) {
				CTxOut treasuryOut(nFees * 0.24 + (nHeight % 100 ? 0 : 200000000000), Params().GetTreasuryRewardScript());
...
void IncrementExtraNonce(CBlock* pblock, CBlockIndex* pindexPrev, unsigned int& nExtraNonce)
{
    ...
    pblock->vtx[0].vin[0].scriptSig = (CScript() << nHeight << CBigNum(nExtraNonce)) + CREDITBASE_FLAGS;
