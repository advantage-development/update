...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    int64_t nSubsidy = 0 * CREDIT;

    if (nHeight == 1) {
        nSubsidy = 1000000000 * CREDIT; // premine
    }
    else if (nHeight > 1 && nHeight <= 100) {
        nSubsidy = 0 * CREDIT; // instamine prevention
    }
        ...
        nSubsidy = 0 * CREDIT; // initial block reward
    }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
	...
	if (pindex->nHeight > 1 && pindex->nHeight % 100 == 0) {nReward += 200000000000; //TreasuryinSats
}
...
bool CTransaction::GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev, uint64_t& nCoinAge) const
{
    ...
    CBigNum bnCoinDay = bnCentSecond * CENT / CREDIT / (24 * 60 * 60);
